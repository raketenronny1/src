...
function [spectra_binned, wavenumbers_binned] = bin_spectra(spectra, wavenumbers, binningFactor)
    ...
    if numOriginalFeatures ~= length(wavenumbers)
        error('bin_spectra:FeatureCountMismatch', ...
              ['Number of spectral features (%d) must match the length of the wavenumbers vector (%d). ', ...
               'Troubleshooting tip: ensure both arrays originate from the same preprocessing step ', ...
               'and apply identical trimming before binning.'], ...
              numOriginalFeatures, length(wavenumbers));
    end
