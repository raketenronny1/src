            ...
            if size(mu,2) ~= size(Xp,2)
                error('apply_model_to_data:DimensionMismatch', ...
                      ['PCAMu length (%d) does not match data width (%d). ', ...
                       'Troubleshooting tip: ensure new spectra are preprocessed with ', ...
                       'the same binning and PCA settings as the training run, or regenerate ', ...
                       'the model artifacts.'], ...
                      size(mu,2), size(Xp,2));
            end
        ...
        otherwise
            if ~isfield(model,'selectedFeatureIndices') || isempty(model.selectedFeatureIndices)
                error('apply_model_to_data:MissingFeatureIndices', ...
                      ['Model is missing selectedFeatureIndices for method %s. ', ...
                       'Troubleshooting tip: confirm the training pipeline saved this field ', ...
                       'and reload the correct trained model file before prediction.'], ...
                      featureMethod);
            end
