...
if ismember('WHO_Grade', analyzeTable.Properties.VariableNames)
    ...
    if iscellstr(whoGradesRaw) || isstring(whoGradesRaw) || iscategorical(whoGradesRaw)
        whoGrades = string(whoGradesRaw);
        if ~isempty(whoGrades)
            keepMask = ismember(whoGrades, ["WHO-1", "WHO-3"]);
        end
