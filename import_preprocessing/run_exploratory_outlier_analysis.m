...
resultsDir = fullfile(projectBasePath, 'results', 'Phase1_ExploratoryOutlierVis_Focused');
figuresDir = fullfile(projectBasePath, 'figures', 'Phase1_ExploratoryOutlierVis_Focused');
...
fprintf('Setup complete. Results saved in:\n  %s\n  %s\n', resultsDir, figuresDir);
...
for i = 1:height(dataTableTrain)
    ...
    Patient_ID_explore_cell{end+1,1} = repmat(dataTableTrain.Diss_ID(i), numIndSpectra, 1); % Keep as cell for now
...
Patient_ID = vertcat(Patient_ID_explore_cell{:}); % Now a cell array of strings
...
fprintf('T2 threshold (alpha=%.2f): %.4f\n', P.alpha_significance, T2_threshold);

X_reconstructed = score(:,1:k_model)*coeff(:,1:k_model)' + mu;
Q_values = sum((X - X_reconstructed).^2, 2);
num_total_pcs = min(size(X,1)-1, size(X,2)); % Max possible PCs
Q_threshold = NaN;
if k_model < num_total_pcs && k_model < length(latent)
    ...
    if theta1>eps && theta2>eps
        h0=1-(2*theta1*theta3)/(3*theta2^2); if h0<=eps,h0=1;end
        ca=norminv(1-P.alpha_significance); val_in_bracket=ca*sqrt(2*theta2*h0^2)/theta1+1+theta2*h0*(h0-1)/(theta1^2);
        if val_in_bracket>0,Q_threshold=theta1*(val_in_bracket)^(1/h0);end
    end
end
if isnan(Q_threshold)||isinf(Q_threshold), Q_threshold=prctile(Q_values,(1-P.alpha_significance)*100); fprintf('Empirical Q-thresh: %.4g\n',Q_threshold);
else, fprintf('Q-SPE thresh (alpha=%.2f): %.4g\n',P.alpha_significance,Q_threshold);end
...
text(T2_threshold,mean(get(gca,'YLim')),sprintf(' T2 Th=%.2f',T2_threshold),'Color',P.colorT2OutlierFlag,'VerticalAlignment','middle','HorizontalAlignment','left','FontSize',P.plotFontSize-2,'BackgroundColor','w','EdgeColor','k','Margin',1);
text(mean(get(gca,'XLim')),Q_threshold,sprintf(' Q Th=%.2g',Q_threshold),'Color',P.colorQOutlierFlag,'VerticalAlignment','bottom','HorizontalAlignment','center','FontSize',P.plotFontSize-2,'BackgroundColor','w','EdgeColor','k','Margin',1);
...
% (Code remains the same as previous response)
fig3 = figure('Name', 'T2 vs Q Raw Distribution by WHO Grade'); fig3.Position = [150 150 750 600]; hold on;
hdl3 = []; leg3 = {}; idx_w1 = (y_numeric == 1); idx_w3 = (y_numeric == 3); idx_unk = ~(idx_w1 | idx_w3);
...
ax4a1=nexttile(tl4a);hold(ax4a1,'on');hdl4a1=[];leg4a1={}; if any(idx_w1),hdl4a1(end+1)=scatter(ax4a1,score(idx_w1,1),score(idx_w1,min(2,size(score,2))),15,P.colorWHO1,'o','filled','MarkerFaceAlpha',0.3);leg4a1{end+1}='WHO-1';end; if any(idx_w3),hdl4a1(end+1)=scatter(ax4a1,score(idx_w3,1),score(idx_w3,min(2,size(score,2))),15,P.colorWHO3,'s','filled','MarkerFaceAlpha',0.3);leg4a1{end+1}='WHO-3';end; if any(idx_unk),hdl4a1(end+1)=scatter(ax4a1,score(idx_unk,1),score(idx_unk,min(2,size(score,2))),15,[0.7 0.7 0.7],'^','filled','MarkerFaceAlpha',0.2);leg4a1{end+1}='Other';end; hold(ax4a1,'off');xlabel(ax4a1,sprintf('PC1(%.1f%%)',explained(1)));ylabel(ax4a1,sprintf('PC2(%.1f%%)',explained(min(2,length(explained)))));title(ax4a1,'2D: PC1 vs PC2','FontWeight','normal');if ~isempty(hdl4a1),legend(ax4a1,hdl4a1,leg4a1,'Location','best','FontSize',P.plotFontSize-2);end;axis(ax4a1,'equal');grid(ax4a1,'on');
ax4a2=nexttile(tl4a);if size(score,2)>=3,hold(ax4a2,'on');hdl4a2=[];leg4a2={}; if any(idx_w1),hdl4a2(end+1)=scatter3(ax4a2,score(idx_w1,1),score(idx_w1,2),score(idx_w1,3),15,P.colorWHO1,'o','filled','MarkerFaceAlpha',0.3);leg4a2{end+1}='WHO-1';end; if any(idx_w3),hdl4a2(end+1)=scatter3(ax4a2,score(idx_w3,1),score(idx_w3,2),score(idx_w3,3),15,P.colorWHO3,'s','filled','MarkerFaceAlpha',0.3);leg4a2{end+1}='WHO-3';end; if any(idx_unk),hdl4a2(end+1)=scatter3(ax4a2,score(idx_unk,1),score(idx_unk,2),score(idx_unk,3),15,[0.7 0.7 0.7],'^','filled','MarkerFaceAlpha',0.2);leg4a2{end+1}='Other';end; hold(ax4a2,'off');view(ax4a2,-30,20);xlabel(ax4a2,sprintf('PC1(%.1f%%)',explained(1)));ylabel(ax4a2,sprintf('PC2(%.1f%%)',explained(2)));zlabel(ax4a2,sprintf('PC3(%.1f%%)',explained(3)));title(ax4a2,'3D: PC1-PC2-PC3','FontWeight','normal');if ~isempty(hdl4a2),legend(ax4a2,hdl4a2,leg4a2,'Location','best','FontSize',P.plotFontSize-2);end;grid(ax4a2,'on');axis(ax4a2,'tight');else,text(0.5,0.5,'<3 PCs','Parent',ax4a2,'HorizontalAlignment','center');title(ax4a2,'3D: PC1-PC2-PC3','FontWeight','normal');end
...
exploratoryOutlierData.flag_Normal_Spectra = is_normal; 
...
exploratoryOutlierData.Patient_ID = Patient_ID; 
