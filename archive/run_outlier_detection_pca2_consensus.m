...
%% --- 8. Identify Outliers (MODIFIED FOR CONSENSUS) ---
disp('8. Identifying outliers based on T2 AND Q (Consensus Strategy)...');
outlier_T2_logical = T2_values > T2_threshold;
outlier_Q_logical = Q_values > Q_threshold;

% --- MODIFIED STRATEGY: Consensus Outliers ---
outlier_indices_logical = outlier_T2_logical & outlier_Q_logical; % Only if BOTH are true

num_outliers = sum(outlier_indices_logical);
% You can still report T2-only, Q-only for informational purposes, but they won't be removed
num_flagged_T2 = sum(outlier_T2_logical);
num_flagged_Q = sum(outlier_Q_logical);

disp([num2str(num_outliers) ' total consensus outliers (T2 AND Q) identified to be removed.']);
disp(['   - Originally flagged by T2: ' num2str(num_flagged_T2)]);
disp(['   - Originally flagged by Q: ' num2str(num_flagged_Q)]);


outlier_indices_numeric = find(outlier_indices_logical); % These are the consensus outliers

if num_outliers > 0
    fprintf('Global indices of CONSENSUS outliers (relative to X_train) to be removed:\n'); disp(outlier_indices_numeric');
    Patient_ID_outliers_consensus = Patient_ID_train(outlier_indices_logical);
    fprintf('Patient IDs of consensus outliers:\n'); disp(unique(Patient_ID_outliers_consensus)'); 
else
    disp('No consensus outliers (T2 AND Q) identified in X_train.');
end
